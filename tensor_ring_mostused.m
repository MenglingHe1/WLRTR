...
ip.addParamValue('Initcore',[], @iscell);
...
%     end
    ...
    if strcmp(Alg,'SGD')
        ...
        %%%%%%%%%%%
        if isempty(Initcore)~=1
        node=Initcore; % Z is the opted cell obtained from last experiment
        end
        %%%%%%%%%%%
        m_t = node;
        v_t = node;
        ...
        while  sqrt(mean(perr.^2)) > Tol
            it = it + 1;
            if mod(it,length(perr)) == 0
                cur=length(perr);
            else
                cur=mod(it,length(perr));
            end
            ...
            for j=1:d
                ...
                if j==1
                    ...
                    sidx = num2cell(I);
                    perr(cur) = chat - c(sidx{:});
                end
                g1 = b'* perr(cur);
                
                if adam ~= 1
                    g2 = g1./(b'.^2 + mu);
                    gradnode{j}(:,I(j),:) = momentum * squeeze(gradnode{j}(:,I(j),:))+ lrate * g2;
                else
                    
                    m_t{j}(:,I(j),:) = beta1 * squeeze(m_t{j}(:,I(j),:)) + (1-beta1) * g1;
                    v_t{j}(:,I(j),:) = beta2 * squeeze(v_t{j}(:,I(j),:)) + (1-beta2) * (g1.^2);
            ...
            for j=1:d
                curnode = node{j}(:,I(j),:);
                grad = gradnode{j}(:,I(j),:);
                curnode =  curnode -  grad;
                node{j}(:,I(j),:) = curnode;
            end
            
            
            if (mod(it,1e5)==0 || it ==1)
         %        tr=tensor_ring;
         %        tr=cell2core(tr,node);
         %       rmse = [rmse,norm(c(:)-full(tr))/sqrt(numel(c))];
        %         err = c(:) - full(tr);
         %        RSE = sqrt(sum(err.^2)/sum(c(:).^2));  
             %   fprintf('it:%d,   err=%6f,RSE=%6f\n',it, sqrt(mean(perr.^2)),RSE);
                 fprintf('it:%d,   err=%6f\n',it, sqrt(mean(perr.^2)));           
            end
            ...
            if (mod(it,1e7)==0)
             ...
             fprintf('Save node to mat file at iter: %d\n\n',it);
                 file_name = [ 'rank',num2str(r(1)),'_Coil1006Diter_',  num2str(it),  '.mat' ];
           ...
           %if it > max(prod(n)*5,1e5)
                disp(['Reaching the required iteration: ',num2str(it)],'\n');
